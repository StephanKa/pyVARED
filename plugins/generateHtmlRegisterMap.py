...
class GenerateHTMLMap(TemplateGeneration):
    ...
    def _format_mask_value(self, bit_mask_value):
        ...
        return f"Decimal: {bit_mask_value}\nHexidecimal: {hex(bit_mask_value)}\nBinary: {bin(bit_mask_value)[2:]}"
    ...
    def _write(self):
        ...
        self.output_file.write(
            f'<!doctype html><html><body><head>{self.CSS_STYLE_SETUP}<h1 align="middle" id="{self.parsed_file.component_name}">{self.parsed_file.component_name} (Version: {self._extract_date_information(self.parsed_file.ip_core_version)})</h1></head>\n{out_string}</body>'
        )
...
class GenerateComponentIndex(GenerateHTMLMap):
    ...
    def _extract_variable_name(self, temp_reg):
        ...
        if temp_reg.variable_name is not None:
    ...
    def _extract_register_name(self, register):
        ...
        for temp_reg in keylist:
            return_string += f'<li style="margin-left:50px;font-family: Arial, Helvetica, sans-serif;font-size:12px"><a href="#{offset_list[temp_reg]}">{hex(temp_reg)} - {offset_list[temp_reg]}</a></li>'
        return return_string
    ...
    def _write_index(self):
        ...
        for temp_component, temp_version, temp_register in self.component_names:
            result_string += f"""<details><summary style="margin-bottom: 10px;margin-top: 20px;font-family: Arial, Helvetica, sans-serif;"><a href="#{temp_component}">
                                <div style="display:inline-block;font-weight: bold;">{temp_component}</div></a><div style="font-size:14px;">
                                (Version: {self._extract_date_information(temp_version)})</div></summary>{self._extract_register_name(temp_register)}</details>"""
