...
class VhdlWriter:
    ...
    def format_string(self):
        ...
        self.write_file(
            alias_definitions=whole_register + partial_register,
            width=self.yaml_object.register_width,
            component_version=whole_date,
            write_process=reset + self.WRITE_PROCESS_ELSE + write_register + self.WRITE_PROCESS_WHEN + save_register,
    ...
    def __format_date(self):
        return self.DATE_DEFINITION.format(
            self.yaml_object.get_name().upper(), self.yaml_object.register_width, self.yaml_object.get_date_information("%y%m%d") + self.yaml_object.get_build_version()
        )
    ...
    def __format_register_read(self, register):
        ...
        if temp_register["Option"]["read"]:
            if temp_register is not None or temp_register != "":
                name = "a_{}".format(temp_register["Name"].replace(" ", "_").lower())
            else:
                name = "slv_reg_{}".format(register[register.find("_") + 1 :])
            return self.READ_DEFINITION.format(
                int(register[register.find("_") + 1 :]), len(self.yaml_object.register_definitions.keys()), name, self.__format_clear_on_read(temp_register, name)
            )
        else:
            return ""
    ...
    def __format_register_write(self, register):
        temp_register = self.yaml_object.register_definitions[register]
        if temp_register["Option"]["write"]:
            return self.WRITE_DEFINITION.format(int(register[register.find("_") + 1 :]), len(self.yaml_object.register_definitions.keys()), register[register.find("_") + 1 :])
        else:
            return ""
    ...
    def __format_register_name(self, register):
        ...
        return self.WHOLE_REG_DEFINITION.format(name, self.yaml_object.register_width, register[register.find("_") + 1 :])
    ...
    def __format_partial_alias(self, register):
        ...
        for temp_bit in temp_register["Bits"].keys():
            ...
            if temp_register is not None or temp_register != "":
                name = "{}_{}".format(temp_register["Name"].replace(" ", "_").lower(), temp_bit.lower())
            else:
                name = "slv_reg" + register[register.find("_") + 1 :]
            if len(bit_definition) == 1:
                bits = bit_definition[0]
            elif len(bit_definition) == 2:
                bits = f"{bit_definition[0]} downto {bit_definition[1]}"
            else:
                raise Exception(f"Bit definition wrong! Please review {register}")
...
if __name__ == "__main__":
    ...
    for dir_path, _dir_names, file_names in list_all_yaml_files:
